...
if submit_to_cluster == False:
  ...
  if check_for_pickle_files:
      if os.path.exists('./mbar.pkl'):
          while 1:
              answer = input("mbar.pkl exists. Would you like to load the file (and skip the initialization of mbar)? y/n\n")
              if answer == 'y':
                  load_pickle = True
                  break
              elif answer == 'n':
...
for perturbation_index in range(nperturbations+1):
    ...
    for target_temperature in range(start_temperature, end_temperature+1, temperature_increment):
        ...
        for i in range (nreduced_bins):
            bin_expectation[i] = mbar.computeExpectations(in_this_bin[i], u_kn=u_kn[perturbation_index], compute_uncertainty=False)[0]
        ...
        E = mbar.computeExpectations(U_kn[perturbation_index], u_kn=u_kn[perturbation_index], compute_uncertainty=False)[0]
        Esquared = mbar.computeExpectations(U_kn[perturbation_index]**2, u_kn=u_kn[perturbation_index], compute_uncertainty=False)[0]
        ...
        if len(reduced_expectation_data) > 0:
            ...
            for i in range(len(reduced_expectation_data)):
                expectation_values.append(mbar.computeExpectations(reduced_expectation_data[i], u_kn=u_kn[perturbation_index], compute_uncertainty=False)[0])
        ...
        #     for i in range(len(reduced_expectation_data)):
        #         fep_values.append(numpy.exp(beta)*mbar.computeExpectations(reduced_fep_data[i], u_kn=u_kn[perturbation_index], compute_uncertainty=False)[0])
        ...
        if compute_per_bin_quantities:
            ...
            for i in range (nreduced_bins):
                ...
                if bin_expectation[i] > precision_threshold:
                    E = mbar.computeExpectations(U_kn[perturbation_index]*in_this_bin[i], u_kn=u_kn[perturbation_index], compute_uncertainty=False)[0]/bin_expectation[i]
                else:
                    E = numpy.nan
            ...
            if len(reduced_expectation_data) > 0:
                ...
                for i in range(nreduced_bins):
                    ...
                    for j in range(len(reduced_expectation_data)):
                        ...
                        # Check to make sure you will actually be able to compute an expectation value
                        if bin_expectation[i] > precision_threshold:
                            E = mbar.computeExpectations(data_in_this_bin, u_kn=u_kn[perturbation_index], compute_uncertainty=False)[0]/bin_expectation[i]
                        else:
                            E = float('nan')
            ...
            if len(reduced_fep_data) > 0:
                ...
                for i in range(nreduced_bins):
                    ...
                    for j in range(len(reduced_fep_data)):
                        ...
                        # Check to make sure you will actually be able to compute an expectation value
                        if bin_expectation[i] > precision_threshold:
                            E = numpy.log(numpy.exp(beta)*mbar.computeExpectations(data_in_this_bin, u_kn=u_kn[perturbation_index], compute_uncertainty=False)[0]/bin_expectation[i])
                            ...
                            fep_values[i][j] = E
                for j in range(len(reduced_fep_data)):
                    min_value = numpy.nanmin(list(zip(*fep_values))[j])
                    for i in range(nreduced_bins):
