...
def number_assign(strength, number):
    ...
    ##strength is the list of number of memories in all, 20 memories needs to be produced.
    ...
    MemInd=[];
    for i in range(len(strength)):
	aaa = (strength[i]*number*1.0)/totalnumber - int((strength[i]*number)/totalnumber);
	print(aaa);
	if aaa >=0.5:
...
def memory_assign(number,wd):
	##number: is the number of memories to be assigned, in our case should be 20
	...
	for ii in range(1,fragnum+1):
		...
		for i in range(len(filelist)):
			if MemInd[i] != 0:
				...
				os.chdir(fragwd);
				q = subprocess.check_output("python ~/opt/script/Pdb2Gro.py %s %s"%(filelist[i][0:-4], namegro),shell=True);
				for j in range(MemInd[i]):
					memfile.write(fragwd+"/"+namegro+" "+str(start1[ii-1])+" "+str(start2[ii-1])+" "+str(length[ii-1])+" "+"1"+"\n");
